...
function StatCard({ title, value, icon: Icon, href, gradient }: {
  ...
  <div className="min-w-0 flex-1 pt-1">
    ...
    <p className={`text-3xl sm:text-4xl font-heading font-bold mt-3 leading-none tabular-nums ${gradient ? "text-white" : ""}`}>{value}</p>
  </div>
  <div className={`w-10 h-10 sm:w-12 sm:h-12 rounded-xl flex items-center justify-center flex-shrink-0 mt-3 ${gradient ? "bg-white/20" : "gradient-primary"}`}>
  ...
  return (
    <div className="p-4 md:p-6 space-y-6 max-w-6xl w-full min-w-0 overflow-x-hidden">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 min-w-0">
        <StatCard title="Appointments" value={appointments.length} icon={CalendarDays} href="/appointments" gradient />
